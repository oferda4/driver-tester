...
TEST(IoctlHandlerTest, InvalidCode) {
    ...
    ASSERT_EQ(IoctlHandler::handle(TestsManager::instance(), arbitraryInvalidCode,
                                   BufferView(nullptr, 0), BufferView(nullptr, 0)),
...
TEST(IoctlHandlerTest, GetNumberOfTests_Sanity) {
    ...
    do {
        ASSERT_EQ(IoctlHandler::handle(TestsManager::instance(), Ioctl::GET_NUMBER_OF_TESTS,
...
TEST(IoctlHandlerTest, ListTests_Sanity) {
    ...
    // the forcing of using only uint8_t is merely for simplifying giving a
    // string name
    ...
    do {
        ...
        ASSERT_EQ(IoctlHandler::handle(
                      TestsManager::instance(), Ioctl::LIST_TESTS,
                      BufferView(&input, sizeof(Ioctl::GetNumberOfTestsInput)),
                      BufferView(output.size() > 0 ? ::std::addressof(output.at(0)) : nullptr,
...
TEST(IoctlHandlerTest, ListTests_BadInput) {
    TestsManager::destroy();
    ASSERT_EQ(IoctlHandler::handle(TestsManager::instance(), Ioctl::LIST_TESTS,
...
TEST(IoctlHandlerTest, ListTests_BadOutput) {
    ...
    ASSERT_EQ(IoctlHandler::handle(
                  TestsManager::instance(), Ioctl::LIST_TESTS, BufferView(&input, sizeof(input)),
...
TEST(IoctlHandlerTest, RunTest_Sanity) {
    ...
    ASSERT_EQ(IoctlHandler::handle(manager, Ioctl::RUN_TEST, BufferView(&input, sizeof(input)),
                                   BufferView(&output, sizeof(output))),
              STATUS_SUCCESS);
...
TEST(IoctlHandlerTest, RunTest_BadInput) {
    TestsManager::destroy();
    ASSERT_EQ(IoctlHandler::handle(TestsManager::instance(), Ioctl::RUN_TEST,
...
TEST(IoctlHandlerTest, RunTest_UnknownTest) {
    ...
    ASSERT_ANY_THROW(IoctlHandler::handle(TestsManager::instance(), Ioctl::RUN_TEST,
                                          BufferView(&input, sizeof(input)),
                                          BufferView(&output, sizeof(output))));
}
