...
TEST(TcpSocketConenctionTest, RecvSanity) {
    ...
    EXPECT_CALL(api.getMock(), recv(ARBITRARY_SOCKET_HANDLE, _, CastUtils::cast<int>(fakeBuffer.size()), 0))
        .Times(1)
    ...
    TcpSocketConnection<MoveableMockPosixTcpSocketAllApi> connection(
        std::move(api),
        SocketGuard<MoveableMockPosixTcpSocketAllApi>(ARBITRARY_SOCKET_HANDLE));
...
TEST(TcpSocketConenctionTest, RecvFails) {
    MoveableMockPosixTcpSocketAllApi api;
    EXPECT_CALL(api.getMock(), recv(_, _, _, _))
        .Times(1)
        .WillOnce(Return(SOCKET_ERROR));
    
    TcpSocketConnection<MoveableMockPosixTcpSocketAllApi> connection(
        std::move(api), 
        SocketGuard<MoveableMockPosixTcpSocketAllApi>(UNINTERESTING_SOCKET_HANDLE));
...
TEST(TcpSocketConenctionTest, SendSanity) {
    const Buffer fakeBuffer = getFakeBuffer();
    MoveableMockPosixTcpSocketAllApi api;
    EXPECT_CALL(api.getMock(), send(ARBITRARY_SOCKET_HANDLE, _, CastUtils::cast<int>(fakeBuffer.size()), 0))
        .Times(1)
...
TEST(TcpSocketConenctionTest, SendFails) {
    MoveableMockPosixTcpSocketAllApi api;
    EXPECT_CALL(api.getMock(), send(_, _, _, _))
        .Times(1)
        .WillOnce(Return(SOCKET_ERROR));
...
TEST(TcpSocketServerTest, Initialization) {
    MoveableMockPosixTcpSocketAllApi api;
    EXPECT_CALL(api.getMock(), create(AF_INET, SOCK_STREAM, IPPROTO_TCP)).Times(1).WillOnce(Return(ARBITRARY_SOCKET_HANDLE));
    EXPECT_CALL(api.getMock(), bind(ARBITRARY_SOCKET_HANDLE, _, sizeof(sockaddr_in))).Times(1).WillOnce(Return(0));
    EXPECT_CALL(api.getMock(), listen(ARBITRARY_SOCKET_HANDLE, 1)).Times(1).WillOnce(Return(0));
    ASSERT_NO_THROW(TcpSocketServer<MoveableMockPosixTcpSocketAllApi>(std::move(api), ARBITRARY_IP_ADDRESS, ARBITRARY_PORT));
}

TEST(TcpSocketServerTest, FailsCreating) {
    MoveableMockPosixTcpSocketAllApi api;
    EXPECT_CALL(api.getMock(), create(AF_INET, SOCK_STREAM, IPPROTO_TCP)).Times(1).WillOnce(Return(INVALID_SOCKET));
    ASSERT_THROW(TcpSocketServer<MoveableMockPosixTcpSocketAllApi>(std::move(api), ARBITRARY_IP_ADDRESS, ARBITRARY_PORT),
...
TEST(TcpSocketServerTest, FailsBinding) {
    ...
    EXPECT_CALL(api.getMock(), bind(ARBITRARY_SOCKET_HANDLE, _, sizeof(sockaddr_in))).Times(1).WillOnce(Return(SOCKET_ERROR));
    ASSERT_THROW(TcpSocketServer<MoveableMockPosixTcpSocketAllApi>(std::move(api), ARBITRARY_IP_ADDRESS, ARBITRARY_PORT),
                 FakeExceptionForMockPosixSocket);
}
...
TEST(TcpSocketServerTest, FailsListening) {
    ...
    EXPECT_CALL(api.getMock(), listen(ARBITRARY_SOCKET_HANDLE, 1)).Times(1).WillOnce(Return(SOCKET_ERROR));
    ASSERT_THROW(TcpSocketServer<MoveableMockPosixTcpSocketAllApi>(std::move(api), ARBITRARY_IP_ADDRESS, ARBITRARY_PORT),
                 FakeExceptionForMockPosixSocket);
}
...
TEST(TcpSocketServerTest, WaitForConnection) {
    ...
    TcpSocketServer<MoveableMockPosixTcpSocketAllApi> server(std::move(api), ARBITRARY_IP_ADDRESS, ARBITRARY_PORT);
    ASSERT_NO_THROW((void)server.waitForConnection());
}

TEST(TcpSocketServerTest, WaitForConnectionFails) {
    MoveableMockPosixTcpSocketAllApi api;
    EXPECT_CALL(api.getMock(), create(_, _, _)).Times(1).WillOnce(Return(UNINTERESTING_SOCKET_HANDLE));
    EXPECT_CALL(api.getMock(), bind(_, _, _)).Times(1).WillOnce(Return(0));
    EXPECT_CALL(api.getMock(), listen(_, _)).Times(1).WillOnce(Return(0));
